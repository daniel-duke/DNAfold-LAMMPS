...
def main():
	...
	parser.add_argument('--avgType',		type=str,	default="last", 	help='how to get hyb times for each staple (values: direct, avg, first, last)')
...
### Heart
	...
	for i in range(nsim):
		...
		if writeIndiv:
			...
			colors = propScafToStap(first_hyb_times_scaled, complements, strands, avgType)
			ars.writeGeo(outGeoFile, dbox3, r, types=strands, charges=colors, bonds=bonds_backbone)
			colors = propScafToStap(hyb_status_allSim[i][:,:n_scaf], complements, strands, avgType)
	...
	if nsim > 1:
		...
		colors = propScafToStap(first_hyb_times_scaled_avg, complements, strands, avgType)
		ars.writeGeo(outGeoFile, dbox3, r, types=strands, charges=colors, bonds=bonds_backbone)
		colors = propScafToStap(hyb_status_avg[:,:n_scaf], complements, strands, avgType)
		writeAtomDump(outDatFile, dbox3, r, colors, dump_every)
		writeOvito(ovitoFile, outGeoFile, outDatFile)
...
def propScafToStap(prop_scaf, complements, strands, avgType):

	### single time step
	if prop_scaf.ndim == 1:
		prop = propScafToStapSingle(prop_scaf, strands, complements, avgType)
	...
	if prop_scaf.ndim == 2:
		...
		for i in range(nstep):
			prop[i] = propScafToStapSingle(prop_scaf[i], strands, complements, avgType)
...
def propScafToStapSingle(prop_scaf, strands, complements, avgType):
	...
	elif avgType == "avg" or avgType == "first" or avgType == "last":
